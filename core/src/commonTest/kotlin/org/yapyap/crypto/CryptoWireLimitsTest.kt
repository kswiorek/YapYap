...
class CryptoWireLimitsTest {
    ...
    fun ratchetCiphertext_decode_rejectsOversizedBodyLength() = runTest {
        ...
        val x3dh = org.yapyap.crypto.e2ee.X3dhHandshake(crypto)
        val (aliceBootstrap, bobBootstrap) = DoubleRatchetSessionTestBootstraps.create(crypto, x3dh)
        val alice = org.yapyap.crypto.e2ee.DoubleRatchetSession.createInitiator(crypto, aliceBootstrap)
...
private object DoubleRatchetSessionTestBootstraps {
    suspend fun create(
        ...
        x3dh: org.yapyap.crypto.e2ee.X3dhHandshake,
    ): Pair<org.yapyap.crypto.e2ee.RatchetBootstrap, org.yapyap.crypto.e2ee.RatchetBootstrap> {
        ...
        val initiator = x3dh.initiatorCompute3Dh(
            local = org.yapyap.crypto.e2ee.X3dhLocalInitiatorKeys(
                identityEncryptionPrivateKey = aliceIk.privateKey,
                identityEncryptionPublicKey = aliceIk.publicKey,
            ),
            remote = org.yapyap.crypto.e2ee.X3dhRemotePeerKeys(
        ...
        val responder = x3dh.responderCompute3Dh(
            local = org.yapyap.crypto.e2ee.X3dhLocalResponderKeys(
