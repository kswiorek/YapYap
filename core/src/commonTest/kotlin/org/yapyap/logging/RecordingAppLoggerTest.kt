...
class RecordingAppLoggerTest {
    ...
    fun recordsDebugInfoWarnError_withFieldsAndThrowable() {
        ...
        log.debug(LogComponent.DATABASE, LoggingTypes.DEDUP_CACHE_HIT, "d", mapOf("a" to 1))
        log.info(LogComponent.CRYPTO, LoggingTypes.STARTED, "i", emptyMap())
        log.warn(LogComponent.ROUTER, LoggingTypes.MESSAGE_NO_PEERS, "w", mapOf("p" to "q"))
        log.error(LogComponent.TOR_TRANSPORT, LoggingTypes.ENVELOPE_HANDLE_FAILED, "e", ex, mapOf())
        ...
        assertEquals(LoggingTypes.DEDUP_CACHE_HIT, log.entries[0].event)

        assertSame(LogLevel.INFO, log.entries[1].level)
        assertEquals(LoggingTypes.STARTED, log.entries[1].event)
    ...
    fun clear_removesEntries() {
        val log = RecordingAppLogger()
        log.info(LogComponent.CRYPTO, LoggingTypes.STARTED, "m", emptyMap())
