...
6. To ensure messages can be delivered when a peer is offline, the messages are simultaneously sent to a number of other peers.
...
- `Orchestrator` (layer) – the top layer of the `core` package, constructs all objects needed for other layers. If not a headless relay, starts `OrchestratorRuntime`.
  ...
  - `OnboardingProvider` exposes the `OnboardingState`, receives the onboarding intro sent by the sonsor (newcomer side)
...
- `Router` (layer) - ensures messages are delvered from an account level to account level.
  ...
  - `PingProvider` sends periodic pings to all known peers ensuring traffic, provides self-reported reliability score and list of chainable UUIDs of common DAGs 
  ...
  - `sync` handles sending and receiving sync requests.
    ...
    - `SyncRetryProcessor` reads the pending sync requests from the DB and retries them periodically trying different peers.
